...
export function ForgotPasswordPage() {
  ...
  const onResetPassword = (e: FormEvent<HTMLFormElement> | undefined) => {
    e?.preventDefault();
    setError("");
    ...
    <div>
      <div className={style.forgot_password_container}>
        ...
        <div className={`${style.links_container}`}>
          <p className={`${style.word} text text_type_main-default`}>
            Вспомнили пароль?
          </p>
          <Link className={style.link} to="/login" state={state}>
            Войти
          </Link>
        </div>
      </div>
