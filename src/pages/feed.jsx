            ...
            <div className='pl-15'>
              <div className={`${style.orders_container} mb-15`}>
                <div className={style.ready_container}>
                  ...
                  {data.length > 0 && message?.orders.slice(0, 5).map((el) => <p className={`${style.text_ready} text text_type_digits-default`} key={el.number}>{el.number}</p>)}
                </div>
                <div className={style.ready_container}>
                  <p className='text text_type_main-medium mb-6'>В работе:</p>
                  {data.length > 0 && message?.orders.filter((el) => el.status != "done").slice(0, 5).map((el) => <p className={`text text_type_digits-default`} key={el.number}>{el.number}</p>)}
                </div>
              </div>
              <div className='mb-15'>
