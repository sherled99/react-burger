import {
  ...
  ADD_TO_BURGER_CONSTRUCTOR,
  CLEAR_BURGER_CONSTRUCTOR,
  DELETE_TO_BURGER_CONSTRUCTOR,
  DELETE_BUN_TO_BURGER_CONSTRUCTOR,
...
interface BurgerState {
  burgerIngredients: Array<IIngredient>;
  burgerIngredientsRequest: boolean;
  burgerIngredientsFailed: boolean;
  burgerConstructor: Array<IIngredientDrop>;
  error: string | null;
}
  ...
  switch (action.type) {
    ...
    case GET_INGREDIENTS_SUCCESS: {
      return {
        ...state,
        burgerIngredientsRequest: false,
        burgerIngredients: action.items,
        burgerIngredientsFailed: false
      };
    }
    ...
    case UPDATE_ORDER_IN_BURGER_CONSTRUCTOR: {
      const fIndex = action.item.index;
